...
export class Dragger extends React.Component {
  static defaultProps = {
    sessionId: Cookie.get('token') || Cookie.get('PHPSESSID'),
    bucket: 'public',
    data: {},
    headers: {},
    rule: {
  ...
  static propTypes = {
    ...
    bucket: PropTypes.oneOf(['public', 'private', 'public-cdn']).isRequired,
  ...
  render() {
    ...
    const props = {
      ...
      action: action || uploadApi[bucket], // action 存在 bucket 设置失效
      ...
      disabled: this.state.fileList.length >= len || disabled,
      directory,
      listType,
      multiple,
      name,
